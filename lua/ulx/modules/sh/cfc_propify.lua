...
local function propifyPlayer( ply, modelPath )
    ...
    prop:Activate()
    ply:SetParent( prop )
    prop:SetVelocity( ply:GetVelocity() )
...
local function propify( caller, targets, modelPath, shouldUnpropify )
    local affectedPlys = {}
...
local propifyCommand = ulx.command( CATEGORY_NAME, "ulx propify", propify, "!propify" )
...
propifyCommand:addParam{ type = ULib.cmds.BoolArg, invisible = true, default = false }
